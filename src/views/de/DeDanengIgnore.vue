<template>
    <div>
        <HeaderDe></HeaderDe>
        <div>
            <h3 style="text-align: center">DE电商-达能商家编码设置</h3>
            <LwBordor></LwBordor>
        </div>
        ...
        methods: {
            refreshOuterId() {
                ...
                this.$http_php.get("/de_gift_ignore.php").then(res => {
                    ...
                    if (res.data.code === 0) {
                        ...
                        console.log(res.data.data.danengOuterIdList, "res.data.danengOuterIdList")
                        _this.outerIdForm.outerIds = [];
                        res.data.data.danengOuterIdList.forEach(function (v, k) {
            ...
            submitOuterId(outerIdForm) {
                ...
                this.$refs['outerIdForm'].validate((valid) => {
                    if (valid) {
                        const _this = this
                        let danengOuterIdList = []
                        outerIdForm.outerIds.forEach(
                            function (v, k) {
                                console.log(v, k, "v,k")
                                danengOuterIdList.push(v.value)
                            }
                        )
